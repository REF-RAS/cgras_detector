...
from cgras.detector.model import DETECT_DAO, CONFIG, SystemConfigNames
...
class YoloModelTable():
    def __init__(self, app, prefix):
        ...
        self.the_panel = html.Div([
                dbc.Row(html.Div([
                    dbc.Button('Edit', id=prefix+'table_edit_button', n_clicks=0, color='secondary', className='mb-1 me-1', size='sm'), 
    ...
    def _table_button_pressed(self): 
        def table_button_pressed(table_edit_button, table_delete_button, model, selected_rows:list):
            if selected_rows is None or len(selected_rows) == 0:
                raise PreventUpdate
            logger.warning(f'selected rows: {selected_rows}')
