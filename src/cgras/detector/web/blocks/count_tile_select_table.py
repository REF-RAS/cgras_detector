...
class CountTileSelectTable():
    ...
    def _row_selected(self):
        def row_selected(active_cell, page_current, page_size):
            if active_cell:
                row = active_cell['row']
                row_actual = row + page_current * page_size
                row_data = self._model.iloc[row_actual]
                # logger.info(f'row_selected: {row_actual} {self._model.iloc[row_actual]}')
                return row_data['tile_id'], None, []
            return None, None, []
        return row_selected
