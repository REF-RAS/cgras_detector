...
class YoloModelFileImportBlock():
    def __init__(self, app, prefix, default_max_end_day=120):
        ...
        self.file_upload_panel = html.Div([
                ...
                html.H4(dbc.Badge('IMPORT COD MODEL SPEC FILE', className='ms-1 me-2', color='white', text_color='secondary')),
    ...
    def _file_import_confirmed(self): 
        def file_import_confirmed(confirm_button, cancel_button, yaml_data):
            ...
            if button_id.endswith('confirm_button'):
                ...
                if result:
                    message = 'Import yolo model file successful'
                    return (True, message, yaml_data, False)                     
                else:
                    message = 'Import yolo model file failed'
