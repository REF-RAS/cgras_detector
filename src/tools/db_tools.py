...
# --- convert a dict into a python object populated with attributes
...
# --- execute sql query that returns a dict of the first row
def query_for_attr_object(db_file, sql, *args) -> dict:
    """ Execute a sql query statement and return the result as a class object of which the fields become attributes

    :param db_file: The path to the sqlite3 db file
    :param sql: The sql query statement
    :return: An object of which the attributes are the fields in the query result
    """
    result = query_for_dict(db_file, sql, args)
    return Dict2Class(result)
...
def query_for_list_of_dicts(db_file, sql, *args):
    """ Execute a sql query statement and return the result as a list of dictionaries

    :param db_file: The path to the sqlite3 db file
    :param sql: The sql query statement
    :return: A list containing objects each of which is a row in the results and the fields are represented as attributes of the object
    """
    with create_connection(db_file) as conn: 
        conn.row_factory = sqlite3.Row
        try:
...
# --- execute sql query that returns a list of objects, each of which represents a record (row) in the results, and the attributes are the fields of the query result
def query_for_list_of_attr_objects(db_file, sql, *args):
    """ Execute a sql query statement and return the result as a list of dobjects each of which
    represents a record (row) in the results, and the attributes are the fields of the query result
    ...
    with create_connection(db_file) as conn: 
        ...
        try:
            results = conn.execute(sql, *args).fetchall()
            result_list = [Dict2Class({k: item[k] for k in item.keys()}) for item in results]
            return result_list
        except Exception as e:
            raise